...
public class Pattern {
    ...
    public Pattern(String format) throws PatternFormatException {
       //TODO: initialize all fields of this class using contents of 
       ...
       //      is similar to that you used in the new ArrayLife constructor
    	...
    	if(inputLength != 7) throw new PatternFormatException("Invalid pattern format: Incorrect number of fields in pattern (found " + inputLength + ").");
    	mName = input[0];
    	mAuthor = input[1];
    	try {
    		...
    		throw new PatternFormatException("Could not interpret the width field as a number: " + numberFormatException.getMessage());
    	}
        mCells = input[6];
    ...
    public void initialise(boolean[][] world) throws PatternFormatException {
       ...
       //      'world' as expressed by the contents of the field 'mCells'.
        ...
        for(int row = 0; row < mStartRow + patternHeight; ++row){
        	if(row < mStartRow) continue;
        	for(int col = 0; col < mStartCol + patternWidth; ++col){
        		if(col < mStartCol) continue;
        		char zeroOne = pattern[row-mStartRow].charAt(col-mStartCol);
        		if(zeroOne=='1')world[row][col] = true;
        		else if(zeroOne=='0') world[row][col] = false;
