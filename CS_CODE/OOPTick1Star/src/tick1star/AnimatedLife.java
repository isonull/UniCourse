...
public class AnimatedLife {
	...
	public boolean getCell(int col, int row) {
		if (row < 0 || row >= mPattern.getHeight())
			return false;
		if (col < 0 || col >= mPattern.getWidth())
			return false;
	...
	private int countNeighbours(int col, int row) {
		...
		if (col >= 0 && col < mPattern.getWidth() && row >= 0 && row < mPattern.getHeight()) {
			...
			n = (col < mPattern.getWidth() - 1 && row < mPattern.getHeight() - 1 && getCell(col + 1, row + 1)) ? ++n
					: n;
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < numberOfImage - 100; ++i) {
			al.nextGeneration();
			image.addFrame(al.getWorld());
		}
